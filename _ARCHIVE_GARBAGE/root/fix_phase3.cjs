...
replaceFile('components/flows/tabs/FlowAnalyticsTab.tsx', [
    [/\? 'đang chạy : 'd.*d\?ng'\}/g, "? 'đang chạy' : 'Đã dừng'}"]
]);
...
replaceFile('pages/Flows.tsx', [
    [/\? 'Kịch bản đang chạy : '/g, "? 'Kịch bản đang chạy' : '"],
    [/\? 'Nếu xóa, hệ thống sẽ ngừng xử lý các email đang chạy : '/g, "? 'Nếu xóa, hệ thống sẽ ngừng xử lý các email đang chạy' : '"],
    [/'đang chạy icon: /g, "'đang chạy', icon: "]
]);
