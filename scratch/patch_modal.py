...
# 2. Update states (Lines 27-39 in original file)
...
# 3. Update the Lead Magnet Block
start_idx = -1
for i, line in enumerate(lines):
    if '<div className="space-y-4 pt-4 border-t border-slate-100">' in line:
        if 'Tính năng "Nhận Voucher qua Form Public"' in lines[i+4]:
            start_idx = i
            break

end_idx = -1
for i in range(start_idx, len(lines)):
    if "{formData.codeType === 'static' ?" in lines[i]:
        end_idx = i
        break
...
lines[start_idx:end_idx] = [magnet_block]
