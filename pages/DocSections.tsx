    ...
    <div className="space-y-20 animate-in fade-in duration-700">
            ...
            <div className="relative z-10 grid grid-cols-1 lg:grid-cols-5 gap-12 items-center">
                <div className="lg:col-span-3 space-y-8">
                    <div className="inline-flex items-center gap-3 px-4 py-2 rounded-xl bg-white/5 border border-white/10">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-[0.3em] text-amber-500">Autoflow Platform Docs v3.0</span>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-4">
                        <button className="px-8 py-4 bg-gradient-to-br from-amber-400 to-amber-600 text-slate-900 font-black rounded-2xl shadow-2xl shadow-amber-500/30 hover:-translate-y-1 transition-all">Bắt đầu ngay</button>
        ...
        <div className="space-y-10">
            <div className="text-center">
                <p className="text-[11px] font-black text-amber-500 uppercase tracking-[0.4em] mb-3">Core Capabilities</p>
                <h2 className="text-4xl font-black text-slate-900">Hệ sinh thái <span className="text-amber-500">toàn diện</span></h2>
            </div>
        ...
        <div className="space-y-10">
            <div className="text-center">
                <p className="text-[11px] font-black text-amber-500 uppercase tracking-[0.4em] mb-3">Onboarding Journey</p>
                <h2 className="text-4xl font-black text-slate-900">Triển khai trong <span className="text-amber-500">3 bước</span></h2>
            </div>
            <div className="grid lg:grid-cols-3 gap-8">
                {[
                    ...
                    { step: '02', title: 'Thiết kế Flow', desc: 'Vẽ hành trình khách hàng bằng Flow Builder. Aura AI viết nội dung cá nhân hóa từng bước.', icon: GitBranch, color: 'bg-amber-500' },
        ...
        <div className="bg-slate-50 rounded-[40px] p-12 border border-slate-100 text-center">
            ...
            <div className="flex flex-wrap justify-center gap-6 lg:gap-10">
                {['Zalo OA', 'Facebook', 'Messenger', 'Gmail', 'Outlook', 'Shopify', 'WooCommerce', 'Lark Base', 'Next.js SDK'].map((b, i) => (
                    <span key={i} className="text-sm font-black text-slate-400 uppercase tracking-widest hover:text-amber-500 transition-colors cursor-default">{b}</span>
                ))}
            </div>
        </div>
    </div>
...
export const SectionCampaigns = () => (
    <div className="space-y-16 animate-in fade-in duration-700">
        <SectionHeader label="Marketing" title={<>Chiến dịch Marketing <span className="text-amber-500">Đẳng cấp</span></>} desc="Gửi Email & ZNS quy mô lớn với targeting chính xác, A/B Testing AI và đo lường sâu." />
        <div className="grid lg:grid-cols-2 gap-10">
            {/* Flow */}
            <div className="space-y-6">
                <div className="bg-slate-50 p-8 rounded-3xl border border-slate-200">
                    <h3 className="text-lg font-black mb-6 flex items-center gap-3"><Target className="w-5 h-5 text-amber-500" />Luồng tạo Chiến dịch</h3>
                    <div className="space-y-8 relative before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-200">
                        {['Lọc Phân khúc "Khách mua trên 2tr" (CDP)', 'Thiết kế Email "Black Friday Private Sale"', 'A/B Test Tiêu đề: "Ưu đãi riêng cho bạn" vs "Giảm 50%"', 'Gửi theo múi giờ cá nhân (Smart Scheduling)', 'Tự động gửi ZNS nhắc nhở nếu chưa Open Email'].map((s, i) => (
                            <div key={i} className="flex gap-5 relative z-10">
                                <div className="w-5 h-5 rounded-full bg-amber-500 border-4 border-white shadow shrink-0 mt-0.5" />
            ...
            <div className="space-y-6">
                <div className="p-8 bg-slate-900 rounded-3xl text-white relative overflow-hidden shadow-2xl">
                    ...
                    <div className="flex items-center justify-between mb-6">
                        <h4 className="font-black text-amber-500 uppercase tracking-widest text-xs">Live A/B Testing</h4>
                    ...
                    {[{ v: 'A (Chủ đề: SALE 50%)', r: '24.2%', c: 'bg-indigo-500', tc: 'text-indigo-400', w: false }, { v: 'B (Chủ đề: Quà tặng riêng)', r: '38.9%', c: 'bg-amber-500', tc: 'text-amber-400', w: true }].map((ab, i) => (
                        <div key={i} className={`p-5 bg-white/5 border ${ab.w ? 'border-amber-500/30' : 'border-white/10'} rounded-2xl mb-4 relative`}>
                            {ab.w && <div className="absolute -top-2 -right-2 bg-amber-500 text-slate-950 text-[8px] font-black px-2 py-0.5 rounded uppercase">Winner</div>}
                    ...
                    <div className="h-28 flex items-end gap-1 mb-4">
                        {[30, 45, 20, 15, 60, 100, 85, 40, 25, 50, 70, 40].map((h, i) => (
                            <div key={i} className="flex-1 relative" style={{ height: `${h}%` }}>
                                <div className={`absolute inset-0 rounded-t ${h === 100 ? 'bg-amber-500' : 'bg-slate-100'}`} />
...
export const SectionAutomation = () => (
    <div className="space-y-16 animate-in fade-in duration-700">
        <SectionHeader label="Automation" title={<>Kịch bản Tự động <span className="text-amber-500">Thông minh</span></>} desc="Visual Flow Builder drag-and-drop: thiết kế hành trình khách hàng không giới hạn." />
        <div className="grid lg:grid-cols-2 gap-10 items-start">
            <div className="space-y-6">
                <div className="p-8 bg-white rounded-3xl border border-slate-100 shadow-sm">
                    <h3 className="font-black text-slate-900 mb-6 flex items-center gap-3"><Zap className="w-5 h-5 text-amber-500" />Loại Triggers</h3>
                    <div className="space-y-3">
                        {[
                ...
                <div className="p-8 bg-slate-900 rounded-3xl text-white">
                    <h4 className="font-black text-amber-500 uppercase tracking-widest text-xs mb-4">Loại Actions</h4>
                    <div className="grid grid-cols-2 gap-3">
                        {['Send Email', 'Send ZNS', 'Send Messenger', 'Add/Remove Tag', 'Update Contact Field', 'Wait (Delay)', 'Condition Split', 'Webhook Call', 'AI Response', 'Score Lead'].map((a, i) => (
                            <div key={i} className="flex items-center gap-2 text-xs text-slate-300 font-bold">
                                <div className="w-1.5 h-1.5 rounded-full bg-amber-500 shrink-0" />{a}
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <div className="p-8 bg-white rounded-3xl border border-slate-100 shadow-xl relative">
                ...
                <div className="space-y-1 flex flex-col items-center">
                    ...
                    <div className="flex items-center gap-3 bg-white border border-slate-100 rounded-full px-5 py-2.5 shadow-sm">
                        <div className="w-8 h-8 rounded-full bg-amber-500 text-white flex items-center justify-center"><Clock className="w-4 h-4" /></div>
        ...
        <div className="p-8 bg-slate-900 rounded-3xl text-white relative overflow-hidden">
            <div className="absolute top-0 right-0 w-32 h-32 bg-amber-500/10 blur-2xl" />
            <h3 className="text-lg font-black text-amber-500 mb-6 flex items-center gap-2"><Clock className="w-5 h-5" /> Chế độ "Wait Until" Thông minh</h3>
...
export const SectionEmailBuilder = () => (
    <div className="space-y-16 animate-in fade-in duration-700">
        <SectionHeader label="Email Builder" title={<>Drag & Drop <span className="text-amber-500">Sáng tạo</span></>} desc="Builder thế hệ mới — WYSIWYG, responsive 100%, tích hợp Aura AI viết nội dung." />
        <div className="grid lg:grid-cols-3 gap-10">
            ...
            <div className="lg:col-span-2 relative rounded-3xl bg-slate-50 border-2 border-slate-200 overflow-hidden min-h-[480px]">
                ...
                <div className="absolute top-10 left-0 bottom-0 w-14 bg-white border-r border-slate-100 flex flex-col items-center py-4 gap-4">
                    {[AlignLeft, Image, Columns, Link2, Sliders].map((Icon, i) => (
                        <div key={i} className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all ${i === 0 ? 'bg-amber-500 text-white shadow-lg' : 'bg-slate-50 border border-slate-100 text-slate-400 hover:bg-slate-100'}`}><Icon className="w-4 h-4" /></div>
                    ))}
                </div>
                <div className="mt-10 ml-14 flex justify-center p-6">
                    <div className="w-full max-w-sm bg-white shadow border border-slate-100 rounded-2xl p-8 space-y-5">
                        ...
                        <div className="h-11 w-full bg-amber-500 rounded-xl shadow shadow-amber-300" />
                    </div>
                </div>
            </div>
            {/* Features */}
            <div className="space-y-4">
                ...
                <div className="p-6 bg-slate-900 rounded-2xl text-white">
                    <h5 className="font-black text-amber-500 uppercase tracking-widest text-[10px] mb-3">Compatibility</h5>
...
export const SectionTemplates = () => (
    <div className="space-y-16 animate-in fade-in duration-700">
        <SectionHeader label="Templates" title={<>Template <span className="text-amber-500">Chuyên nghiệp</span></>} desc="Thư viện hàng trăm mẫu email được thiết kế bởi chuyên gia, sẵn dùng ngay." />
        ...
        <div className="grid lg:grid-cols-2 gap-8">
            <div className="p-8 bg-slate-900 rounded-3xl text-white">
                <h3 className="font-black text-amber-500 uppercase tracking-widest text-xs mb-5">Template Variables</h3>
...
export const SectionAudience = () => (
    <div className="space-y-16 animate-in fade-in duration-700">
        <SectionHeader label="Customer Data Platform" title={<>Quản lý <span className="text-amber-500">Khách hàng 360°</span></>} desc="CDP hợp nhất dữ liệu từ mọi điểm chạm — website, store, ads, social — vào một hồ sơ duy nhất." />
        {/* Contact table mock */}
        <div className="bg-white rounded-3xl border border-slate-100 shadow-xl overflow-hidden">
            <div className="p-6 border-b border-slate-50 bg-slate-50/50 flex items-center justify-between">
                <div className="flex items-center gap-4"><div className="w-10 h-10 rounded-xl bg-amber-500 text-white flex items-center justify-center shadow-lg shadow-amber-500/20"><Users className="w-5 h-5" /></div><h4 className="font-black text-slate-900">Contacts CDP</h4></div>
                    ...
                    <tbody className="divide-y divide-slate-50">
                        {[
                            ...
                            <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td className="px-6 py-4"><div className="flex items-center gap-2"><div className="h-1.5 w-16 bg-slate-100 rounded-full overflow-hidden"><div className={`h-full rounded-full ${row.score > 70 ? 'bg-emerald-500' : row.score > 30 ? 'bg-amber-500' : 'bg-rose-500'}`} style={{ width: `${row.score}%` }} /></div><span className="text-xs font-black text-slate-700">{row.score}</span></div></td>
        ...
        <div className="grid lg:grid-cols-2 gap-8">
            <div className="p-8 bg-white border border-slate-100 rounded-3xl">
                <h3 className="font-black text-slate-900 mb-5 flex items-center gap-3"><Filter className="w-5 h-5 text-amber-500" />Segment Builder</h3>
                <div className="space-y-3">
            ...
            <div className="p-8 bg-slate-900 rounded-3xl text-white">
                <h3 className="font-black text-amber-500 uppercase tracking-widest text-xs mb-5">Import Nguồn dữ liệu</h3>
                <div className="space-y-3">
                    {['Excel / CSV upload (bounding 500k rows)', 'Facebook Lead Ads sync tự động', 'API POST /contacts/import', 'Zalo form sync', 'Website tracking merge by email/phone', 'Manual add one by one'].map((s, i) => (
                        <div key={i} className="flex items-center gap-3 text-sm text-slate-300"><ArrowRight className="w-4 h-4 text-amber-500 shrink-0" />{s}</div>
                    ))}
                </div>
            </div>
        </div>
...
export const SectionTags = () => (
    <div className="space-y-16 animate-in fade-in duration-700">
        <SectionHeader label="Tags & Segmentation" title={<>Hệ thống <span className="text-amber-500">Phân loại</span></>} desc="Tags linh hoạt + Auto-tagging rules để phân loại khách hàng điền vào đúng flow." />
        <div className="grid lg:grid-cols-2 gap-10">
            <div className="space-y-6">
                <div className="p-8 bg-white border border-slate-100 rounded-3xl">
                    <h3 className="text-lg font-black mb-6 flex items-center gap-3"><Tag className="w-5 h-5 text-amber-500" />Tags phổ biến</h3>
                    <div className="space-y-3">
                        {[{ name: 'Potential Lead', count: '1,452', color: 'bg-blue-500' }, { name: 'VIP Member', count: '243', color: 'bg-amber-500' }, { name: 'Cart Abandoned', count: '3,892', color: 'bg-rose-500' }, { name: 'Completed Course', count: '852', color: 'bg-emerald-500' }, { name: 'Whale Customer', count: '128', color: 'bg-violet-500' }].map((tag, i) => (
            ...
            <div className="p-8 bg-slate-900 text-white rounded-3xl relative overflow-hidden shadow-2xl">
                ...
                <h3 className="font-black text-amber-500 uppercase tracking-widest text-xs mb-8 relative z-10">Smart Auto-Tagging Rules</h3>
...
export const SectionAITraining = () => (
    <div className="space-y-16 animate-in fade-in duration-700">
        <SectionHeader label="AI Training — RAG Engine" title={<>Huấn luyện <span className="text-amber-500">Chuyên gia AI</span></>} desc="Upload kiến thức, AI học và trả lời chính xác 24/7 — thay bạn chốt đơn và chăm sóc khách hàng." />
        <div className="grid lg:grid-cols-2 gap-10">
            <div className="p-8 bg-white border border-slate-200 rounded-3xl shadow-sm">
                <div className="flex items-center gap-5 mb-8"><div className="w-14 h-14 rounded-3xl bg-amber-500 flex items-center justify-center text-white shadow-xl shadow-amber-500/20"><Cpu className="w-7 h-7" /></div><div><h3 className="text-xl font-black text-slate-900">Nguồn kiến thức</h3><p className="text-[10px] uppercase font-bold text-slate-400 tracking-widest">RAG Engine v2.4 (Active)</p></div></div>
                <div className="space-y-4">
                    {[
                        ...
                        <div key={i} className="group p-5 bg-slate-50 rounded-2xl border border-slate-100 flex items-center gap-4 cursor-pointer hover:bg-white hover:shadow-lg hover:border-amber-200 transition-all">
                            <div className="w-11 h-11 rounded-xl bg-white flex items-center justify-center text-slate-400 group-hover:bg-amber-500 group-hover:text-white transition-all shadow-sm"><src.icon className="w-5 h-5" /></div>
            ...
            <div className="space-y-6">
                <div className="p-8 bg-slate-950 rounded-3xl text-white relative overflow-hidden shadow-2xl">
                    ...
                    <h3 className="text-xl font-black mb-6 flex items-center gap-3 relative z-10"><Globe2 className="w-6 h-6 text-amber-500" />Multi-channel Connect</h3>
