...
replaceStr('components/flows/modals/StepParticipantsModal.tsx', "border-slate-100 hover:border-slate-200'}\`}>Đang chờ", "border-slate-100 hover:border-slate-200'}\`}>Đang chờ</button>");
replaceStr('components/flows/tabs/FlowAnalyticsTab.tsx', "Automation {currentFlow.status === 'active' ? 'Đang chờ : 'd d?ng'}", "Automation {currentFlow.status === 'active' ? 'Đang chờ' : 'Đã dừng'}");
...
replaceStr('pages/Flows.tsx', "{ id: 'active', label: 'Đang chờ icon: PlayCircle },", "{ id: 'active', label: 'Đang chờ', icon: PlayCircle },");
