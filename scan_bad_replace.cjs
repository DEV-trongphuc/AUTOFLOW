...
["e:/AUTOFLOW/AUTOMATION_FLOW/components/", "e:/AUTOFLOW/AUTOMATION_FLOW/pages/"].forEach(dir => {
    walkDir(dir, file => {
        ...
        const matches = content.match(/.{0,20}Đang chờ.{0,20}/g) || [];
        matches.forEach(m => {
            // Flag if "Đang chờ" appears inside a word/string not as standalone text
            if (!/[>"\s]Đang chờ[\s<",(]/.test(m) && !/^\s*Đang chờ\s*$/.test(m.trim())) {
...
console.log(broken.length === 0 ? "No mid-word Đang chờ found!" : broken.join("\n"));
