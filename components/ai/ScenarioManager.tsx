            ...
            <div className="flex items-start gap-4 p-4 pl-5">
                ...
                <div className="flex items-center gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <button onClick={onToggle} title={scenario.is_active ? 'Tắt kịch bản' : 'Bật kịch bản'}
                        className={`p-1.5 rounded-lg transition-all ${scenario.is_active
                            ? (isDark ? 'text-emerald-400 hover:bg-emerald-500/10' : 'text-emerald-500 hover:bg-emerald-50')
                            : (isDark ? 'text-slate-500 hover:bg-slate-700' : 'text-slate-300 hover:bg-slate-100')
                        }`}>
                        {scenario.is_active ? <ToggleRight className="w-4 h-4" /> : <ToggleLeft className="w-4 h-4" />}
                    </button>
                    ...
                    <div className="space-y-1.5">
                        ...
                        <input
                            ...
                            className={`w-full h-11 px-4 border rounded-xl text-sm font-medium outline-none transition-all
                                ${isDark ? 'bg-slate-800 border-slate-700 text-slate-200 focus:border-amber-500/50' : 'bg-white border-slate-200 text-slate-800 focus:border-amber-400'}`}
                        />
                    </div>
                                 ...
                                 <div className="mb-4">
                                             ...
                                             <div key={tIdx} className="relative">
                                                 <textarea
                                                     ...
                                                     className={`w-full px-3 py-2 border rounded-lg text-sm outline-none resize-y min-h-[80px] max-h-[400px]
                                                         ${isDark ? 'bg-slate-900 border-slate-600 text-slate-200 focus:border-amber-500/50' : 'bg-slate-50 border-slate-200 text-slate-800 focus:border-amber-400'}
                                                         ${arr.length > 1 ? 'pr-9' : ''}`}
                                                 />
                                                 {arr.length > 1 && (
                                                     <button
                                     ...
                                     <button
                                         ...
                                         className={`inline-flex items-center gap-1.5 text-[11px] font-bold mt-2 px-3 py-1.5 rounded-lg border border-dashed transition-all
                                             ${isDark ? 'border-amber-500/30 text-amber-400 hover:bg-amber-500/10' : 'border-amber-200 text-amber-600 hover:bg-amber-50'}`}
                                     >
...
const ScenarioManager: React.FC<Props> = ({ propertyId, isDarkTheme, brandColor }) => {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className={`flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 rounded-2xl border
                ...
                <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <div className={`flex items-center gap-3 px-4 py-2 rounded-xl border cursor-pointer select-none transition-all
                        ...
                        <div className={`w-10 h-5 rounded-full p-0.5 transition-all duration-300 flex items-center
            ...
            {!globalEnabled && (
                <div className={`flex items-center gap-3 px-5 py-4 rounded-xl border ${isDark ? 'border-amber-500/20 bg-amber-500/5' : 'border-amber-200 bg-amber-50'}`}>
