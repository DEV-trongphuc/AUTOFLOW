        ...
        <div className="space-y-2 p-3 bg-slate-50 rounded-2xl border border-slate-100">
            <div className="flex justify-between items-center">
                ...
                <div className="flex bg-white border border-slate-200 rounded-lg p-0.5 scale-90 origin-right">
                    ...
                    {!forcedUnit && (<><button onClick={() => handleUpdate(`${displayValue}px`)} className={`p-1 rounded transition-all ${currentUnit === 'px' && !isAuto ? 'bg-amber-500 text-white' : 'text-slate-400 hover:text-slate-600'}`}><PxIcon className="w-3 h-3" /></button><button onClick={() => handleUpdate(`${displayValue}%`)} className={`p-1 rounded transition-all ${currentUnit === '%' && !isAuto ? 'bg-amber-500 text-white' : 'text-slate-400 hover:text-slate-600'}`}><Percent className="w-3 h-3" /></button></>)}
                </div>
            </div>
            {!isAuto && (
                <div className="flex items-center gap-3">
                    {!hideSlider && <div className="flex-1 relative h-6 flex items-center">
                        <input type="range" min="0" max={currentUnit === '%' ? 100 : max} value={isNaN(displayValue) ? 0 : displayValue} onChange={(e) => handleUpdate(`${e.target.value}${currentUnit}`)} className="w-full h-1.5 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-amber-500" />
                    </div>}
                    <div className={`relative ${hideSlider ? 'w-full' : 'w-14'} shrink-0`}>
                        <input type="number" value={isNaN(displayValue) ? 0 : displayValue} onChange={(e) => handleUpdate(`${e.target.value}${currentUnit}`)} className="w-full text-right text-[10px] font-black text-slate-700 bg-white border border-slate-200 rounded-md py-1 pr-5 pl-1 outline-none focus:border-amber-500" />
