...
import {
    ...
    ShieldCheck, Code, FileCode, Eye, Save, ChevronRight, ChevronLeft, Calendar, Send, GitMerge, Layout, Braces, ChevronDown, Loader2, ArrowRight, Zap, Sparkles, Smartphone, FileText, Download, Upload
} from 'lucide-react';
...
const CampaignWizard: React.FC<CampaignWizardProps> = ({
    ...
    const handlePublishClick = async () => {
        // P0 FIX: Prevent double-click with immediate lock
        if (publishLockRef.current || isSubmitting) return;

        // [VALIDATION] Server-side Fresh Check for Linked Flow Status
        // We MUST verify the REAL status of the flow before allowing "Send Now"
        if (!formData.scheduledAt) {
            try {
                // Fetch fresh list of flows
                const resFlows = await api.get<Flow[]>('flows');
                if (resFlows.success && Array.isArray(resFlows.data)) {
                    const freshFlows = resFlows.data;
                    let flowToCheck: Flow | undefined;

                    if (activateFlowId) {
                        flowToCheck = freshFlows.find(f => f.id === activateFlowId);
                    } else if (formData.id) {
                        // Find implicitly linked flow (where Trigger = Campaign THIS_ID)
            ...
            <div className={`relative w-full lg:max-w-6xl bg-[#fdfdfd] shadow-2xl h-full flex flex-col border-l border-slate-100 transform transition-all duration-500 cubic-bezier(0.16, 1, 0.3, 1) ${animateWizardIn ? 'translate-x-0 opacity-100' : 'translate-x-full lg:translate-x-[100px] opacity-0'}`}>
                    ...
                    {step === 1 && (
                        <TabTransition className="space-y-6 max-w-4xl mx-auto py-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                >
                                    <div className={`absolute top-0 right-0 w-32 h-32 rounded-bl-[100px] transition-all duration-700 ${(!formData.type || formData.type === 'email') ? 'bg-blue-500/10' : 'bg-slate-50 opacity-0 group-hover:opacity-100'}`}></div>
                                        ...
                                        <div className="flex justify-between items-center text-xs mt-1.5 px-1">
                                            ...
                                            {(formData.subject?.length || 0) > 60 ? (
                                                <span className="text-orange-600 font-semibold flex items-center gap-1">
                                                    ⚠️ Gmail hiển thị tối đa 60 ký tự
                                                </span>
                                ...
                                <div className="w-full">
                                    {formData.templateId === 'custom-html' ? (
                                        ...
                                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                            ...
                                            {/* Right: Live Preview */}
                                            <div className="bg-slate-100 rounded-[24px] p-4 border border-slate-200 shadow-sm">
                ...
                <div className="px-8 py-5 bg-white border-t border-slate-200 flex justify-between items-center shrink-0 z-20">
                    ...
                    <div className="flex gap-3">
                        <Button
                            variant="secondary"
                            onClick={() => { onSaveDraft(formData); onClose(); }}
                            icon={Save}
                            className="px-6 h-11 rounded-xl text-[11px] font-bold uppercase hidden sm:flex"
                            disabled={formData.status === CampaignStatus.SENT || formData.status === CampaignStatus.SENDING}
                        >
                            Lưu nháp
                        </Button>
