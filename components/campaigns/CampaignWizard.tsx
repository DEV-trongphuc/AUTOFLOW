    ...
    return createPortal(
        <>
            <div className="fixed inset-0 z-[150] flex justify-end">
                ...
                />
                <div className={`relative w-full lg:max-w-[calc(100vw-260px)] bg-[#fdfdfd] shadow-2xl h-full flex flex-col border-l border-slate-100 transform transition-all duration-500 cubic-bezier(0.16, 1, 0.3, 1) ${animateWizardIn ? 'translate-x-0 opacity-100' : 'translate-x-full lg:translate-x-[100px] opacity-0'}`}>
                            ...
                            {steps.map(s => {
                                ...
                                return (
                                    <div key={s.id} onClick={() => handleStepClick(s.id)} className={`flex items-center group ${done || active ? 'cursor-pointer' : 'cursor-not-allowed'}`}>
                                        <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all duration-300 ${active ? 'bg-violet-600 shadow-lg shadow-violet-500/20 scale-110' : (done ? 'bg-emerald-500 hover:bg-emerald-600' : 'bg-slate-100 text-slate-400')}`}>
                    ...
                    <div className="px-8 py-5 bg-white border-t border-slate-100 flex justify-between items-center shrink-0">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            {step < 5 ? (
                                <Button
                                    onClick={handleNext}
                                    className="bg-gradient-to-r from-violet-600 to-indigo-600 text-white px-8 py-3 rounded-xl shadow-lg shadow-violet-500/20 hover:scale-105 transition-all font-bold group"
                                >
                                    TIẾP TỤC
