...
const UIDTip = () => (
    <div className="bg-blue-600/10 border border-blue-500/20 p-5 rounded-[24px] space-y-3 animate-in slide-in-from-left-4 duration-500 delay-300">
        ...
        <div className="space-y-2">
            <p className="text-[11px] text-slate-600 font-medium leading-relaxed">
                Gửi ZNS qua <span className="text-blue-600 font-bold">UID</span> (Subscriber) giúp bạn <span className="text-blue-600 font-bold">tiết kiệm chi phí</span> hơn so với gửi qua SĐT và tăng khả năng phản hồi trực tiếp ngay trong khung chat OA.
            </p>
            <div className="flex items-start gap-2 bg-white/60 p-2.5 rounded-xl border border-blue-100">
                <ReceiptText className="w-3.5 h-3.5 text-blue-500 shrink-0 mt-0.5" />
    ...
    return (
        <div className="relative" ref={ref}>
            <button
                ...
                className="w-8 h-8 flex items-center justify-center text-slate-300 hover:text-blue-500 hover:bg-blue-50 rounded-lg transition-all"
            >
                ...
                <div className={`absolute right-0 mt-2 w-48 bg-white border border-slate-100 rounded-xl shadow-2xl z-[10020] py-2 transform transition-all duration-200 ${animatePickerIn ? 'opacity-100 scale-100' : 'opacity-0 scale-95 translate-y-2'}`}>
                    ...
                    {MERGE_TAGS.map(tag => (
                        <button
                            ...
                            className="w-full px-4 py-2 text-left text-xs font-bold text-slate-700 hover:bg-blue-50 hover:text-blue-600 transition-colors"
                        >
                            {tag.label}
                        </button>
                    ))}
    ...
    return (
        <div className="relative w-full" ref={containerRef}>
            <button
                type="button"
                onClick={() => setIsOpen(!isOpen)}
                className={`w-full px-4 py-3 flex items-center justify-between bg-white border rounded-[18px] text-sm font-semibold transition-all ${isOpen ? 'border-blue-500 ring-4 ring-blue-50' : 'border-slate-200 hover:border-blue-400'}`}
            >
                ...
                <div className={`absolute z-[10010] w-full mt-2 bg-white border border-slate-100 rounded-[22px] shadow-[0_20px_50px_rgba(0,0,0,0.1)] max-h-60 overflow-auto transform transition-all duration-200 ${animateDropdownIn ? 'opacity-100 scale-100' : 'opacity-0 scale-95 translate-y-2'}`}>
                    {options.length > 0 ? options.map((option) => (
                        <button
                            ...
                            className={`w-full px-5 py-3.5 flex items-start text-left hover:bg-blue-50 transition-colors group ${option.value === value ? 'bg-blue-50/50' : ''}`}
                        >
                            <div className="flex-1 min-w-0">
                                <p className={`text-sm font-bold truncate ${option.value === value ? 'text-blue-700' : 'text-slate-700'}`}>{option.label}</p>
                                {option.subLabel && <p className="text-[10px] text-slate-400 mt-1 truncate font-medium">{option.subLabel}</p>}
                            </div>
                            {option.value === value && <Check className="w-4 h-4 text-blue-600 mt-0.5 ml-2 shrink-0" />}
                        </button>
                    )) : (
            ...
            <div
                ...
                <div className="flex-1 flex overflow-hidden">
                    ...
                    <div className="w-[480px] h-full overflow-y-auto px-10 pb-10 space-y-6 custom-scrollbar">
                                ...
                                <CustomSelect
                                    ...
                                    options={[{
                                        ...
                                        icon: activeOA?.avatar ? (
                                            ...
                                            <div className="w-6 h-6 rounded-full bg-blue-100 flex items-center justify-center text-[10px] font-bold text-blue-600">I</div>
                                        )
                                    }]}
                                    onChange={() => { }}
                        ...
                        {selectedTemplate && (
                            <>
                                ...
                                <div className="bg-[#111827] rounded-[32px] p-6 shadow-xl space-y-4 text-white relative overflow-hidden animate-in slide-in-from-left-4 duration-500">
                                    ...
                                    <div className="grid grid-cols-2 gap-4 relative z-10">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xl font-bold text-blue-400">~ {znsPrice.toLocaleString()}<span className="text-[10px] ml-0.5 text-blue-300">đ/tin</span></p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="bg-orange-50/50 border border-orange-200 p-5 rounded-[24px] space-y-3 animate-in slide-in-from-left-4 duration-500 delay-75">
                                    ...
                                    <div className="space-y-2">
                                        <p className="text-[11px] text-slate-600 font-medium leading-relaxed">
                                            Theo chính sách Zalo, tin nhắn ZNS chỉ được gửi trong khung giờ: <span className="text-orange-700 font-bold">06:00 - 22:00</span> hàng ngày.
                                        </p>
                                        <div className="flex items-start gap-2 bg-white/60 p-2.5 rounded-xl border border-orange-100">
                                ...
                                {/* CSV ACTIONS */}
                                <div className="bg-white p-6 rounded-[32px] border-2 border-dashed border-slate-200 bg-blue-50/5 space-y-4 animate-in slide-in-from-left-4 duration-500 delay-150">
                                    <div className="flex items-center gap-3">
                                        <div className="p-2 bg-blue-50 rounded-xl text-blue-600 border border-blue-100"><FileSpreadsheet className="w-5 h-5" /></div>
                                        <div>
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        ...
                                        <button onClick={() => fileInputRef.current?.click()} className="flex items-center justify-center gap-2 py-2.5 px-3 bg-blue-50 hover:bg-blue-100 text-blue-600 rounded-xl text-[10px] font-bold transition-all border border-blue-100 uppercase tracking-widest">
                                ...
                                <div className="bg-white p-6 rounded-[32px] border border-slate-200 shadow-sm space-y-6 animate-in slide-in-from-left-4 duration-500 delay-200">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <div className="px-2 py-0.5 bg-blue-100/50 rounded-full text-[9px] font-bold text-blue-600 uppercase tracking-tighter">UID INPUT</div>
                                    </div>
                                        ...
                                        {selectedTemplate.preview_data.map(p => (
                                            <div key={p.name} className="space-y-1.5 group">
                                                ...
                                                <div className="relative">
                                                    <input
                                                        type="text"
                                                        className="w-full text-xs p-3.5 pr-14 rounded-2xl border border-slate-200 bg-slate-50/20 focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-50/50 transition-all outline-none font-semibold text-slate-700"
                    ...
                    <div className="flex-1 bg-[#f1f5f9]/50 rounded-tl-[64px] border-l border-t border-slate-200 flex justify-center items-center relative overflow-hidden group p-10">
                        ...
                        <div className="absolute -top-24 -right-24 w-[500px] h-[500px] bg-blue-100/40 rounded-full blur-[120px] pointer-events-none"></div>
                        ...
                        {selectedTemplate ? (
                                            ...
                                            {isLoadingDetail ? (
                                                ...
                                                <div className="w-full h-full bg-white overflow-hidden relative">
                                                    ...
                                                    <button
                                                        ...
                                                        className="absolute bottom-4 right-4 p-2 bg-white/80 backdrop-blur-sm border border-slate-200 rounded-full text-slate-400 hover:text-blue-500 hover:border-blue-200 transition-all shadow-sm z-50 pointer-events-auto"
                            ...
                            <div className="text-center space-y-6 animate-in fade-in zoom-in-95 duration-700">
                                <div className="w-[100px] h-[100px] bg-white rounded-[32px] shadow-2xl flex items-center justify-center mx-auto ring-1 ring-slate-100"><Zap className="w-12 h-12 text-blue-500 animate-pulse" /></div>
                                <div>
                ...
                <div className="px-12 py-8 border-t border-slate-50 flex justify-between items-center bg-white shrink-0 z-50">
                    <div className="flex items-center gap-10">
                        <div className="flex items-center gap-4">
                            <div className="w-12 h-12 rounded-2xl bg-blue-50 text-blue-600 flex items-center justify-center text-xl font-bold border border-blue-100/50">{activeRecipients.length}</div>
