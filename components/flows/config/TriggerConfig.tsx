...
const TriggerConfig: React.FC<TriggerConfigProps> = ({ config, onChange, disabled, locked }) => {
    ...
    const triggerOptions = [
        { id: 'segment', label: 'Phân khúc d?ng', icon: Layers, color: 'orange', desc: 'B? l?c thông minh' },
        { id: 'form', label: 'G?i Bi?u m?u', icon: FileInput, color: 'amber', desc: 'T? Landing Page' },
        ...
        { id: 'zalo_follow', label: 'Quan tâm Zalo', icon: UserPlus, color: 'cyan', desc: 'Khi khách nh?n Follow' },
        { id: 'custom_event', label: 'Custom Event', icon: Zap, color: 'violet', desc: 'S? ki?n tùy ch?nh' },
        { id: 'tag', label: 'Ðu?c g?n nhãn', icon: Tag, color: 'emerald', desc: 'Phân lo?i th? công' },
        { id: 'date', label: 'Ngày / S? ki?n', icon: Calendar, color: 'blue', desc: 'Sinh nh?t, Ng? dông' },
        { id: 'campaign', label: 'Sau Chi?n d?ch', icon: Send, color: 'indigo', desc: 'Tuong tác Email' },
        { id: 'unsubscribe', label: 'H?y dang ký', icon: UserMinus, color: 'red', desc: 'Khi khách nh?n Unsub' },
    ];
        ...
        switch (type) {
            case 'segment':
                ...
                if (subtype === 'sync') {
                    ...
                    return list ? `Ð?ng b?: ${list.name}` : 'Khi d?ng b? d? li?u';
                }
                const list = lists.find(l => l.id === targetId);
                return list ? `Vào Danh sách: ${list.name}` : 'Khi vào Danh sách';
            case 'form':
                const form = forms.find(f => f.id === targetId);
                return form ? `G?i Form: ${form.name}` : 'Khi g?i Bi?u m?u';
            case 'purchase':
            ...
            case 'custom_event':
                ...
                return ce ? `S? ki?n: ${ce.name}` : 'Khi có s? ki?n tùy ch?nh';
            case 'inbound_message':
                return targetId ? `Tin nh?n: "${targetId}"` : 'Khi khách g?i tin nh?n';
            case 'zalo_follow':
                return 'Khi khách Quan tâm Zalo OA';
            case 'unsubscribe':
                return 'Khi khách H?y dang ký';
            case 'tag':
                return targetId ? `Ðu?c g?n Tag: ${targetId}` : 'Khi du?c g?n nhãn';
            case 'campaign':
                const camp = campaigns.find(c => c.id === targetId);
                return camp ? `Sau Campaign: ${camp.name}` : 'Tuong tác chi?n d?ch';
            case 'date':
                if (dateField === 'dateOfBirth' || dateField === 'anniversaryDate' || dateField === 'joinedAt') {
                    ...
                    const attr = isBirth ? 'sinh nh?t' : (isJoined ? 'ngày gia nh?p' : 'ngày k? ni?m');
                    const offsetType = config.offsetType || 'on';
                    const offsetVal = config.offsetValue || 0;
                    if (offsetType === 'on') return `M?ng ${isBirth ? 'Sinh nh?t' : (isJoined ? 'Ngày gia nh?p' : 'K? ni?m')}`;
                    return `${offsetVal} ngày ${offsetType === 'before' ? 'tru?c' : 'sau'} ${attr}`;
                }
                if (dateField === 'specificDate') return `Vào ngày ${config.specificDate || '...'}`;
                if (dateField === 'lastActivity') return 'Khách hàng ng? dông';
                if (dateField === 'custom_field_date') {
                    const fieldKey = config.customFieldKey || '...';
                    const offsetType = config.offsetType || 'on';
                    const offsetVal = config.offsetValue || 0;
                    if (offsetType === 'on') return `Trigger ngày: ${fieldKey}`;
                    return `${offsetVal} ngày ${offsetType === 'before' ? 'tru?c' : 'sau'} [${fieldKey}]`;
                }
                return 'S? ki?n theo ngày';
        ...
        <div className="space-y-6 animate-in fade-in duration-500 pb-32">
            {locked && (
                <div className="p-4 bg-amber-50 border border-amber-200 rounded-[22px] flex items-center gap-4 shadow-sm animate-in zoom-in-95 mb-4">
                    ...
                    <div>
                        <p className="text-[11px] font-bold text-amber-800 uppercase tracking-tight">Trigger dã b? khóa</p>
                        <p className="text-[10px] font-medium text-amber-700 leading-tight">Quy trình dã có khách tham gia, không th? thay d?i di?m b?t d?u.</p>
                    </div>
                </div>
            )}
            ...
            {/* 2. Ð?NG B?: DETAILED CONFIGURATION */}
            <div className="space-y-4 pt-4 border-t border-slate-100">
                <div className="flex items-center justify-between px-1">
                    ...
                    {triggerType === 'form' && config.targetId && (
                        <button onClick={() => setShowGuide(true)} className="text-[9px] font-bold text-blue-600 hover:underline">Hu?ng d?n tích h?p API</button>
                    )}
                </div>
                    ...
                    <div className="space-y-3.5 animate-in slide-in-from-bottom-2 duration-300">
                        ...
                        {triggerType === 'segment' && (
                            <div className="flex bg-slate-100 p-0.5 rounded-lg w-full mb-1">
                                ...
                                >
                                    ...
                                    <span className="ml-1">Ð?ng b?</span>
                                </button>
                                <button
                        ...
                        {triggerType !== 'date' && (
                            <div className="relative group">
                                ...
                                <input value={searchTerm} onChange={e => setSearchTerm(e.target.value)} placeholder="Tìm ki?m nhanh..." className="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-xs font-semibold outline-none focus:border-slate-400 transition-all" />
                            </div>
                        )}
                        ...
                        <div className={`space-y-2 pr-1 custom-scrollbar p-0.5 ${triggerType === 'date' ? '' : 'max-h-72 overflow-y-auto'}`}>
                            ...
                            {triggerType === 'segment' && targetSubtype === 'sync' && lists
                                ...
                                .map(item => (
                                    <ConfigItem
                                        key={item.id}
                                        label={item.name}
                                        desc={`${item.count || 0} liên h? • ${item.source}`}
                            ...
                            {triggerType === 'form' && forms
                                ...
                                .map(f => (
                                    <ConfigItem
                                        key={f.id}
                                        label={f.name}
                                        desc={`${f.stats?.submissions || 0} lu?t dang ký`}
                                        icon={FileInput}
                            ...
                            {triggerType === 'tag' && tags
                                ...
                                .map(t => (
                                    <ConfigItem
                                        key={t.id}
                                        label={t.name}
                                        desc={`Kích ho?t khi g?n nhãn này`}
                                        icon={Tag}
                                ...
                                <div className="space-y-4 animate-in slide-in-from-bottom-2 duration-300 p-1">
                                    <div className="p-5 bg-white border border-slate-200 rounded-[24px] space-y-4 shadow-sm">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <span className="text-[9px] font-bold uppercase text-slate-400 tracking-wider">T? khóa kích ho?t (Tùy ch?n)</span>
                                        </div>
                                        <div className="space-y-2">
                                            <input
                                                type="text"
                                                placeholder="Ví d?: GIÁ, TU VAN, BAO GIA"
                                            ...
                                            />
                                            <p className="text-[10px] text-slate-400 font-medium">Nh?p t? khóa khách g?i. Ð? tr?ng n?u mu?n kích ho?t cho M?I tin nh?n.</p>
                                        </div>
                                    </div>
                                    <div className="p-4 bg-blue-50/30 rounded-2xl border border-blue-100 flex items-start gap-3">
                                        <AlertCircle className="w-4 h-4 text-blue-400 shrink-0 mt-0.5" />
                                        <p className="text-[11px] text-blue-600 font-medium leading-relaxed">
                                            H? th?ng s? ki?m tra tin nh?n c?a khách trên Meta (Messenger) và Zalo OA.
                                            N?u kh?p v?i t? khóa (không phân bi?t hoa thu?ng), Flow s? du?c kích ho?t ngay l?p t?c.
                                        </p>
                                    </div>
                                </div>
                            )}
                            ...
                            {triggerType === 'zalo_follow' && (
                                <div className="p-8 text-center space-y-4 animate-in zoom-in-95 duration-300">
                                    ...
                                    <h3 className="text-sm font-bold text-slate-800">Kích ho?t khi Quan tâm Zalo OA</h3>
                                    <p className="text-[11px] text-slate-500 max-w-[240px] mx-auto leading-relaxed">
                                        T? d?ng g?i tin nh?n chào m?ng ho?c b?t d?u quy trình cham sóc ngay khi khách hàng nh?n <b>"Quan tâm"</b> Zalo OA c?a b?n.
                                    </p>
                                    <div className="p-3 bg-slate-50 rounded-xl inline-block border border-slate-100">
                                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Không c?n c?u hình thêm</span>
                                    </div>
                                </div>
                            )}
                            ...
                            {triggerType === 'unsubscribe' && (
                                <div className="p-8 text-center space-y-4 animate-in zoom-in-95 duration-300">
                                    ...
                                    <h3 className="text-sm font-bold text-slate-800">Kích ho?t khi H?y dang ký</h3>
                                    <p className="text-[11px] text-slate-500 max-w-[240px] mx-auto leading-relaxed">
                                        S? d?ng d? g?n nhãn "Ng?ng quan tâm" ho?c ghi nh?n ph?n h?i vào CRM khi khách hàng nh?n link H?y dang ký trong Email.
                                    </p>
                            ...
                            {triggerType === 'date' && (
                                <div className="space-y-3">
                                    {/* 1. KHÁCH HÀNG NG? ÐÔNG (Ðua lên trên) */}
                                    <ConfigItem
                                        label="Khách hàng ng? dông"
                                        desc="Khi khách không tuong tác quá lâu"
                                        icon={Snowflake}
                                    ...
                                    {config.dateField === 'lastActivity' && (
                                        <div className="p-5 bg-white border border-slate-200 rounded-[24px] space-y-4 animate-in slide-in-from-top-2 shadow-sm mb-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="text-[9px] font-bold uppercase text-slate-400 tracking-wider">Th?i gian không tuong tác</span>
                                            </div>
                                            <div className="flex gap-4 items-center">
                                                <input
                                                    ...
                                                    disabled={disabled}
                                                />
                                                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Ngày liên t?c</span>
                                            </div>
                                            <div className="p-3.5 bg-blue-50/50 rounded-xl flex items-start gap-2.5 border border-blue-100/50">
                                                <Info className="w-3.5 h-3.5 text-blue-500 shrink-0 mt-0.5" />
                                                <p className="text-[10px] text-blue-700 font-medium leading-relaxed italic">H? th?ng s? quét d?nh k? nh?ng khách hàng không M? ho?c Click link trong {config.inactiveAmount || 30} ngày d? dua vào lu?ng.</p>
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    {/* 2. SINH NH?T */}
                                    <ConfigItem
                                        label="Chúc m?ng Sinh nh?t"
                                        desc="Ch?y vào ngày sinh ho?c tru?c/sau"
                                        icon={Cake}
                                    ...
                                    {/* 3. K? NI?M */}
                                    <ConfigItem
                                        label="M?ng ngày K? ni?m"
                                        desc="Theo ngày k? ni?m riêng c?a khách"
                                        icon={Calendar}
                                    ...
                                    {/* 4. NGÀY GIA NH?P */}
                                    <ConfigItem
                                        label="K? ni?m Ngày gia nh?p"
                                        desc="Ch?y hàng nam vào ngày khách dang ký"
                                        icon={History}
                                    ...
                                    {/* 5. NGÀY C? TH? */}
                                    <ConfigItem
                                        label="Ngày c? th? (L?, S? ki?n)"
                                        desc="Ch?y vào m?t ngày c? d?nh duy nh?t"
                                        icon={Clock}
                                    ...
                                    {config.dateField === 'specificDate' && (
                                        <div className="p-5 bg-white border border-slate-200 rounded-[24px] space-y-4 animate-in slide-in-from-top-2 shadow-sm mb-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="text-[9px] font-bold uppercase text-slate-400 tracking-wider">Ch?n ngày di?n ra s? ki?n</span>
                                            </div>
                                            <div className="flex gap-4 items-center">
                                                <input
                                                    type="date"
                                            ...
                                            <div className="p-3.5 bg-blue-50/50 rounded-xl flex items-start gap-2.5 border border-blue-100/50">
                                                <Info className="w-3.5 h-3.5 text-blue-500 shrink-0 mt-0.5" />
                                                <p className="text-[10px] text-blue-700 font-medium leading-relaxed italic">Ví d?: Thi?t l?p ngày 14/02 d? b?t d?u chuong trình Valentine cho toàn b? khách hàng du?c ch?n.</p>
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    <ConfigItem
                                        ...
                                        desc="Tru?c/sau ngày luu trong tru?ng tùy ch?nh"
                                        icon={ArrowRight}
                                        ...
                                        <div className="p-5 bg-white border border-violet-200 rounded-[24px] space-y-4 animate-in slide-in-from-top-2 shadow-sm mb-4">
                                            ...
                                            <div className="space-y-2">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="text-[9px] font-bold uppercase text-slate-400 tracking-wider">Tru?ng ngày (Custom Field Key)</span>
                                                </div>
                                                {customFieldDefs.length > 0 ? (
                                                    <select
                                                        ...
                                                        disabled={disabled}
                                                    >
                                                        <option value="">-- Ch?n tru?ng ngày --</option>
                                                    ...
                                                    <input
                                                        type="text"
                                                        placeholder="Nh?p key, ví d?: ngay_dat_lich"
                                                ...
                                                <p className="text-[9px] text-slate-400">Key c?a custom field ch?a giá tr? ngày (YYYY-MM-DD ho?c DD/MM/YYYY)</p>
                                            </div>
                                            ...
                                            <div className="space-y-2">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="text-[9px] font-bold uppercase text-slate-400 tracking-wider">Th?i di?m kích ho?t</span>
                                                </div>
                                                <div className="grid grid-cols-4 gap-2">
                                                    <Select
                                                        label="Ki?u"
                                                        options={[
                                                            { value: 'before', label: 'Tru?c X ngày' },
                                                            { value: 'on', label: 'Ðúng ngày' },
                                                ...
                                                <p className="text-[9px] italic text-slate-400 bg-slate-50 p-2 rounded-lg border border-slate-100">
                                                    {(() => {
                                                        ...
                                                        if (config.offsetType === 'before') return `Kích ho?t ${x} ngày TRU?C [${field}] lúc ${hh}:${mm}`;
                                                        if (config.offsetType === 'after') return `Kích ho?t ${x} ngày SAU [${field}] lúc ${hh}:${mm}`;
                                                        return `Kích ho?t dúng ngày [${field}] lúc ${hh}:${mm}`;
                                                    })()}
                                                </p>
                                            </div>
                                            ...
                                            <div className="space-y-2">
                                                                    ...
                                                                    >
                                                                        <div className="flex items-center gap-2">
                                                                            ...
                                                                            <div>
                                                                                ...
                                                                                <p className="text-[9px] text-slate-400">{item.count || 0} liên h?</p>
                                                                            </div>
                                                                        </div>
                                                ...
                                                <div className="p-3 bg-violet-50/50 rounded-xl flex items-start gap-2.5 border border-violet-100">
                                                    ...
                                                    <p className="text-[10px] text-violet-700 font-medium leading-relaxed">
                                                        Cron job hàng ngày quét subscriber có field <b>{config.customFieldKey || 'ngày'}</b> kh?p di?u ki?n và t? d?ng enroll vào flow.
                                                    </p>
                                                </div>
                                            </div>
                                        </div>
                                            ...
                                            <div className="p-5 bg-white border border-slate-200 rounded-[24px] space-y-4 animate-in slide-in-from-top-2 shadow-sm">
                                                <div className="flex items-center justify-between">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <span className="text-[9px] font-bold uppercase text-slate-400 tracking-wider">Ð?i tu?ng áp d?ng</span>
                                                    </div>
                                                </div>
                                                    ...
                                                    <div className="mt-4 space-y-4">
                                                        <div className="p-3 bg-amber-50 rounded-xl flex items-start gap-2.5 border border-amber-200/50">
                                                            ...
                                                            <p className="text-[10px] text-amber-700 font-bold leading-relaxed">
                                                                Ch? kích ho?t theo ngày d?i v?i nh?ng ngu?i có trong danh sách du?c ch?n du?i dây.
                                                                <span className="block font-medium text-slate-500 mt-0.5 mt-0.5 italic">N?u h? không thu?c danh sách này, flow s? không t? d?ng b?t d?u.</span>
                                                            </p>
                                                        </div>
                                                        ...
                                                        <div className="relative group mb-2">
                                                            ...
                                                            <input
                                                                ...
                                                                placeholder="Tìm ki?m danh sách..."
                                                        ...
                                                        <div className="mt-3 px-1 flex items-center justify-between border-t border-slate-100 pt-2">
                                                            <span className="text-[10px] font-semibold text-slate-500">U?c tính (Unique):</span>
                                                            <div className="flex items-center gap-1.5">
                                                                ...
                                                                <span className="text-[10px] font-bold text-slate-700">
                                                                    {isCalculating ? (
                                                                        ...
                                                                        `~${estimatedCount !== null ? estimatedCount : 0} liên h?`
                                                                    )}
                                                                </span>
                                                            </div>
                                                        </div>
