...
const ConditionConfig: React.FC<ConditionConfigProps> = ({ config, onChange, flow, stepId, disabled }) => {
    ...
    const getActionDescription = () => {
        switch (config.conditionType) {
            ...
            case 'unsubscribed': return 'h?y dang ký';
                ...
                <>
                    ...
                    <Radio
                        ...
                        options={parentEmailStep?.type === 'zalo_zns' ? [
                            { id: 'zns_delivered', label: 'Ðã nh?n (G?i thành công)', icon: CheckSquare, desc: 'Tin nh?n dã g?i thành công' },
                            ...
                            { id: 'delivered', label: 'Ðã nh?n (Delivered)', icon: MailCheck, desc: 'N?u KHÔNG -> Chuy?n nhánh ELSE' },
                            ...
                            ...((parentEmailStep?.type === 'trigger' && parentEmailStep?.config?.type === 'campaign') ? [
                                { id: 'received_reminder', label: 'Ðã nh?n Reminder', icon: Bell, desc: 'Ðã nh?n Email Nh?c nh?' },
                                { id: 'opened_reminder', label: 'Ðã m? Reminder', icon: MailOpen, desc: 'Ðã m? Email Nh?c nh?' }
                            ] : []),
                            { id: 'unsubscribed', label: 'H?y dang ký', icon: UserMinus, desc: 'Phân lo?i khách r?i di' },
                        ]}
                                ...
                                <>
                                    ...
                                    {!disabled && selectedLinks.length === availableLinks.length && (
                                        <div className="p-3 bg-amber-50 border border-amber-200 rounded-2xl flex items-start gap-3 animate-in fade-in zoom-in duration-300">
                                            ...
                                            <p className="text-[10px] text-amber-700 font-bold leading-relaxed lowercase first-letter:uppercase">Ðang ? ch? d? "B?t k? Link nào". B? ch?n m?t link b?t k? n?u b?n ch? mu?n theo dõi danh sách c? th?.</p>
                                        </div>
                                    )}
