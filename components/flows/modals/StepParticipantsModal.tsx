...
const StepParticipantsModal: React.FC<StepParticipantsModalProps> = ({
        ...
        try {
            ...
            if (res.success && res.data) {
                ...
                if (currentStep) {
                    ...
                    if (type === 'condition') {
                        branches.push(
                            { id: currentStep.yesStepId || currentStep.yesStepID, label: '? YES', description: 'Ði?u ki?n dúng' },
                            { id: currentStep.noStepId || currentStep.noStepID, label: '? NO', description: 'Ði?u ki?n sai' }
                        );
    ...
    const executeBulkAction = async (targetStepId: string | null, executeAction: boolean) => {
        ...
        try {
            ...
            if (res.success) {
                ...
                const action = executeAction ? 'hoàn thành bu?c này' : 'b? qua';
                toast.success(`Ðã ${action} cho ${count} khách hàng`);
                setSelectedIds(new Set());
                setIsGlobalSelected(false);
                onRefresh();
                onActionComplete?.();
            } else {
                toast.error(res.message || 'Có l?i x?y ra');
            }
        } catch (error) {
            toast.error('Không th? th?c hi?n thao tác');
        } finally {
    ...
    const executeRemove = async () => {
        ...
        try {
            ...
            if (res.success) {
                ...
                toast.success(`Ðã lo?i ${actualCount} khách hàng ra kh?i flow`);
                setSelectedIds(new Set());
                setIsGlobalSelected(false);
                onRefresh();
                onActionComplete?.();
    ...
    const executeResend = async () => {
        ...
        try {
            ...
            if (res.success) toast.success(res.message || 'Ðã g?i l?i thành công');
    ...
    const handleBulkAddTag = async () => {
        ...
        try {
            ...
            if (res.success) {
                const count = isGlobalSelected ? pagination.total : selectedIds.size;
                toast.success(`Ðã g?n tag cho ${count} khách hàng`);
                setSelectedIds(new Set());
                setIsGlobalSelected(false);
    ...
    const handleBulkAddToList = async () => {
        if (!selectedList || (selectedIds.size === 0 && !isGlobalSelected)) {
            toast.error('Vui lòng ch?n danh sách');
            return;
        }
        ...
        try {
            ...
            if (res.success) {
                const count = isGlobalSelected ? pagination.total : selectedIds.size;
                toast.success(`Ðã thêm ${count} khách hàng vào danh sách`);
                setSelectedIds(new Set());
                setIsGlobalSelected(false);
    ...
    const handleBulkChangeStatus = async () => {
        if (!selectedStatus || (selectedIds.size === 0 && !isGlobalSelected)) {
            toast.error('Vui lòng ch?n tr?ng thái');
            return;
        }
        ...
        try {
            ...
            if (res.success) {
                const count = isGlobalSelected ? pagination.total : selectedIds.size;
                toast.success(`Ðã c?p nh?t tr?ng thái cho ${count} khách hàng`);
                setSelectedIds(new Set());
                setIsGlobalSelected(false);
    ...
    return (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center px-4 !mt-0">
            ...
            >
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-500 font-medium">
                                T?ng s?: {pagination.total} {['clicks', 'zns_clicked'].includes(activeTab) ? 'lu?t click' : 'khách hàng'}
                                {selectedIds.size > 0 && ` • ${selectedIds.size} dã ch?n`}
                            </p>
                        </div>
                    </div>
                                ...
                                onClick={() => {
                                    ...
                                    if (stepData) {
                                        ...
                                        if (type === 'condition' || type === 'advanced_condition') {
                                            branches.push(
                                                { id: stepData.yesStepId || stepData.yesStepID, label: 'Nhánh YES (Ði?u ki?n dúng)' },
                                                { id: stepData.noStepId || stepData.noStepID, label: 'Nhánh NO (Ði?u ki?n sai)' }
                                            );
                                        } else if (type === 'split_test') {
                    ...
                    <div className="flex items-center gap-2">
                        {stepType === 'action' && (
                            ...
                            >
                                ...
                                <span>{activeTab === 'preview' ? 'Ðóng Xem Mail' : 'XEM MAIL'}</span>
                            </button>
                        )}
                        <button onClick={onRefresh} className="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Làm m?i">
                ...
                {stepType === 'zalo_zns' && (
                    <div className="px-6 py-3 border-b border-slate-50 flex gap-2 overflow-x-auto" style={{ minHeight: '56px' }}>
                        <button onClick={() => onTabChange('zns_sent')} className={`px-3 py-1.5 rounded-lg text-[10px] font-bold transition-all border ${activeTab === 'zns_sent' ? 'bg-blue-50 text-blue-600 border-blue-200' : 'bg-white text-slate-500 border-slate-100 hover:border-slate-200'}`}>G?i thành công</button>
                        <button onClick={() => onTabChange('waiting')} className={`px-3 py-1.5 rounded-lg text-[10px] font-bold transition-all border ${activeTab === 'waiting' ? 'bg-amber-50 text-amber-600 border-amber-200' : 'bg-white text-slate-500 border-slate-100 hover:border-slate-200'}`}>Ðang ch?</button>
                        <button onClick={() => onTabChange('zns_clicked')} className={`px-3 py-1.5 rounded-lg text-[10px] font-bold transition-all border ${activeTab === 'zns_clicked' ? 'bg-cyan-50 text-cyan-600 border-cyan-200' : 'bg-white text-slate-500 border-slate-100 hover:border-slate-200'}`}>Ðã Click Link</button>
                        <button onClick={() => onTabChange('zns_replied')} className={`px-3 py-1.5 rounded-lg text-[10px] font-bold transition-all border ${activeTab === 'zns_replied' ? 'bg-violet-50 text-violet-600 border-violet-200' : 'bg-white text-slate-500 border-slate-100 hover:border-slate-200'}`}>Ðã Ph?n h?i</button>
                            ...
                            <thead className="bg-slate-50/80 border-b border-slate-200 text-left sticky top-0 z-20 backdrop-blur-sm">
                                {selectedIds.size > 0 && ['waiting', 'failed', 'unsubscribed', 'zns_failed', 'inactive'].includes(activeTab) ? (
                                        ...
                                        <th colSpan={['waiting', 'failed', 'unsubscribed', 'zns_failed', 'inactive'].includes(activeTab) ? 5 : 4} className="px-6 py-3">
                                            <div className="flex items-center justify-between w-full">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <span className="text-xs font-bold text-slate-700">Ðã ch?n <span className="text-orange-600 font-black text-sm">{selectedIds.size}</span> khách hàng</span>
                                                </div>
                                                <div className="flex items-center gap-2">
                                                    {stepType !== 'inactive' && (
                                                        <>
                                                            ...
                                                            >
                                                                ...
                                                                <span>{bulkActionLoading ? 'Ðang g?i...' : 'Complete & Next'}</span>
                                                            </button>
                                                            <button
                                                            ...
                                                            >
                                                                ...
                                                                <span>{bulkActionLoading ? 'Ðang g?i...' : 'B? qua & Next'}</span>
                                                            </button>
                                                            ...
                                                            {quickActionsOpen && (
                                                                <div className="absolute top-full right-0 mt-2 w-56 bg-white rounded-xl shadow-xl border border-slate-200 py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                                                                    ...
                                                                    >
                                                                        ...
                                                                        <div>
                                                                            <p className="text-xs font-bold text-slate-700">Ð?i Tr?ng thái</p>
                                                                            <p className="text-[10px] text-slate-400">C?p nh?t tr?ng thái</p>
                                                                        </div>
                                                                    </button>
                                                                </div>
                                                            )}
                                    ...
                                    <tr>
                                        ...
                                        {stepType === 'completed' ? (
                                            <>
                                                <th className="px-6 py-3 text-xs font-bold text-slate-500 uppercase tracking-wider">Bu?c cu?i cùng</th>
                                                <th className="px-6 py-3 text-xs font-bold text-slate-500 uppercase tracking-wider text-center">Tr?ng thái</th>
                                            ...
                                            <>
                                                <th className="px-6 py-3 text-xs font-bold text-slate-500 uppercase tracking-wider w-[200px]">Tr?ng thái</th>
                                                {activeTab === 'opened' && (
                                                    <>
                                                        <th className="px-6 py-3 text-xs font-bold text-slate-500 uppercase tracking-wider text-center">S? L?N</th>
                                                ...
                                                {activeTab === 'waiting' ? (
                                                    <>
                                                        <th className="px-6 py-3 text-xs font-bold text-slate-500 uppercase tracking-wider">TH?I GIAN VÀO</th>
                            ...
                            <tbody className="divide-y divide-slate-50 relative">
                                ...
                                {selectedIds.size === participants.length && participants.length > 0 && pagination.total > participants.length && ['waiting', 'failed', 'unsubscribed', 'zns_failed', 'zns_skipped'].includes(activeTab) && (
                                    <tr className="bg-orange-50/50">
                                        <td colSpan={['waiting', 'failed', 'unsubscribed', 'zns_failed', 'inactive', 'zns_skipped'].includes(activeTab) ? 5 : 4} className="px-6 py-2.5 text-center">
                                            {isGlobalSelected ? (
                                                <p className="text-xs font-medium text-slate-600">Ðã ch?n t?t c? <span className="font-bold text-orange-600">{pagination.total.toLocaleString()}</span> khách hàng. <button onClick={() => setIsGlobalSelected(false)} className="ml-2 text-blue-600 font-bold hover:underline">B? ch?n</button></p>
                                            ) : (
                                                <p className="text-xs font-medium text-slate-600">Ðã ch?n {participants.length} khách hàng. <button onClick={() => setIsGlobalSelected(true)} className="ml-1 text-orange-600 font-bold hover:underline italic underline-offset-2">Ch?n t?t c? {pagination.total.toLocaleString()} khách hàng?</button></p>
                                            )}
                                        </td>
                                    </tr>
                                )}
                                ...
                                {loading && participants.length === 0 ? (
                                    ...
                                    <tr><td colSpan={['waiting', 'failed', 'unsubscribed', 'zns_failed', 'inactive', 'zns_skipped'].includes(activeTab) ? 6 : 5} className="py-20 text-center">
                                        <p className="text-xs font-bold text-slate-400">Không tìm th?y d? li?u nào.</p>
                                    </td></tr>
                                                ...
                                                <>
                                                    <td className="px-6 py-3">
                                                        {p.status === 'completed' ? (
                                                            ...
                                                            <span className="bg-blue-50 text-blue-600 px-2 py-1 rounded-md text-[10px] font-bold border border-blue-100 inline-flex items-center gap-1">
                                                                <MessageSquare className="w-3 h-3" /> ZNS dã g?i
                                                            </span>
                                                            ...
                                                            <span className="bg-emerald-50 text-emerald-600 px-2 py-1 rounded-md text-[10px] font-bold border border-emerald-100 inline-flex items-center gap-1">
                                                                <Check className="w-3 h-3" /> Ðã kh?p
                                                            </span>
                                                        ) : p.status === 'condition_false' ? (
                                                            <span className="bg-rose-50 text-rose-600 px-2 py-1 rounded-md text-[10px] font-bold border border-rose-100 inline-flex items-center gap-1">
                                                                <X className="w-3 h-3" /> Không kh?p
                                                            </span>
                                                            ...
                                                            <span className="bg-violet-50 text-violet-600 px-2 py-1 rounded-md text-[10px] font-bold border border-violet-100 inline-flex items-center gap-1">
                                                                <RefreshCcw className="w-3 h-3 animate-spin" /> Ðang x? lý
                                                            </span>
                                                            ...
                                                            <span className="bg-amber-50 text-amber-600 px-2 py-1 rounded-md text-[10px] font-bold border border-amber-100 inline-flex items-center gap-1">
                                                                <Clock className="w-3 h-3" /> Ðang d?i
                                                            </span>
                                                        )}
                                                    </td>
                                                                ...
                                                                (() => {
                                                                    ...
                                                                    if (stepType === 'condition' && (activeTab as string) === 'waiting' && stepConfig?.waitDuration) {
                                                                        ...
                                                                        if (diff > 0) {
                                                                            ...
                                                                            const m = Math.floor((diff % (1000 * 60 * 60)) / (1000 * 60));
                                                                            return (
                                                                                <span className="inline-flex items-center gap-1 font-bold text-blue-600" title={`Ki?m tra m?i 1 phút. H?t h?n lúc: ${new Date(deadline).toLocaleString()}`}>
                                                                                    <Clock className="w-3.5 h-3.5" />
            ...
            <ConfirmModal
                ...
                title="Xác nh?n lo?i b?"
                message={`B?n có ch?c mu?n lo?i ${isGlobalSelected ? pagination.total : selectedIds.size} khách hàng ra kh?i flow?`}
                confirmText="Lo?i b?"
                cancelText="H?y"
                variant="danger"
                ...
                <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl border border-slate-100 animate-in fade-in zoom-in-95 duration-200">
                        ...
                        <div className="p-6 space-y-4">
                            <div>
                                <label className="block text-xs font-bold text-slate-600 mb-2 uppercase tracking-wide">
                                    Danh sách Email / SÐT / Zalo UID (M?i dòng 1 thông tin)
                                </label>
                                <textarea
                                ...
                                <p className="mt-2 text-xs text-slate-400">
                                    * Nh?ng liên h? chua có trong h? th?ng s? du?c t? d?ng t?o m?i.
                                </p>
                            </div>
                            ...
                            {availableBranches.length > 0 && (
                                <div>
                                                ...
                                                {(() => {
                                                    ...
                                                    return (
                                                        <>
                                                            ...
                                                            <span className="text-blue-700">{selectedBranch?.label || 'Ðã ch?n nhánh'}</span>
                                                        </>
                                                    );
                                                })()}
                                            </div>
                                                ...
                                                <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-xl shadow-xl border border-slate-100 p-2 z-50 animate-in fade-in zoom-in-95 duration-200 ring-4 ring-slate-400/10 max-h-[300px] overflow-y-auto custom-scrollbar">
                                                    ...
                                                    <div className="px-3 pb-2 pt-1">
                                                        <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">
                                                            B?t bu?c vào nhánh (B? qua logic)
                                                        </span>
                                                    </div>
                                    ...
                                    <p className="mt-2 text-xs text-amber-600 bg-amber-50 p-2 rounded-lg border border-amber-100 flex gap-2">
                                        ...
                                        <span>
                                            N?u ch?n nhánh c? th?, ngu?i dùng s? <b>b? qua</b> logic c?a bu?c này và di th?ng vào nhánh dã ch?n.
                                        </span>
                                    </p>
                                </div>
                            )}
                        ...
                        <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-3 rounded-b-2xl">
                            ...
                            <button
                                onClick={() => {
                                    if (!addEmails.trim()) return toast.error('Vui lòng nh?p ít nh?t 1 liên h?');
                                    setAddingUsers(true);
                                    ...
                                    api.post(`flows?id=${flowId}&route=manual-add-participant`, {
                                        ...
                                        if (res.success) {
                                            toast.success(res.message || `Ðã thêm ${data?.added || 0} khách hàng`);
                                            setAddEmails(''); // Clear input
                                            setIsAddModalOpen(false); // Close modal
                                            onRefresh(); // Refresh list AND stats
                                        } else {
                                            toast.error(res.message || 'L?i thêm khách hàng');
                            ...
                            >
                                ...
                                {addingUsers ? 'Ðang thêm...' : 'Thêm ngay'}
                            </button>
                        </div>
                    </div>
                </div>
                ...
                <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-300">
                    <div className="bg-white w-full max-w-md rounded-[24px] shadow-2xl border border-slate-100 p-6 animate-in zoom-in-95 duration-300">
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            <div>
                                <h3 className="text-base font-bold text-slate-800">G?n Tag khách hàng</h3>
                                <p className="text-xs text-slate-500">Ch?n tag mu?n g?n cho {isGlobalSelected ? pagination.total : selectedIds.size} khách hàng</p>
                            </div>
                        </div>
                    ...
                    <div className="bg-white w-full max-w-md rounded-[24px] shadow-2xl border border-slate-100 p-6 animate-in zoom-in-95 duration-300">
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-slate-500">Ch?n list mu?n thêm {isGlobalSelected ? pagination.total : selectedIds.size} khách hàng vào</p>
                            </div>
                        </div>

                        <div className="space-y-4">
                            <div>
                                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 block">Ch?n Danh sách</label>
                                <select
                            ...
                            <div className="flex gap-3 pt-2">
                                ...
                                >
                                    {bulkActionLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Check className="w-4 h-4" />}
                                    <span>Xác nh?n thêm vào List</span>
                                </button>
                            </div>
                        </div>
                    </div>
                ...
                <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-300">
                    <div className="bg-white w-full max-w-md rounded-[24px] shadow-2xl border border-slate-100 p-6 animate-in zoom-in-95 duration-300">
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            <div>
                                <h3 className="text-base font-bold text-slate-800">Ð?i tr?ng thái liên h?</h3>
                                <p className="text-xs text-slate-500">C?p nh?n tr?ng thái cho {isGlobalSelected ? pagination.total : selectedIds.size} khách hàng</p>
                            </div>
                        </div>

                        <div className="space-y-4">
                            <div>
                                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 block">Ch?n tr?ng thái m?i</label>
                                <div className="grid grid-cols-2 gap-2">
                                    {[
                                        { value: 'active', label: 'Ho?t d?ng', bg: 'bg-emerald-50 text-emerald-600 border-emerald-100' },
                                        { value: 'unsubscribed', label: 'H?y dang ký', bg: 'bg-orange-50 text-orange-600 border-orange-100' },
                            ...
                            <div className="flex gap-3 pt-2">
                                ...
                                >
                                    {bulkActionLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Check className="w-4 h-4" />}
                                    <span>Xác nh?n d?i tr?ng thái</span>
                                </button>
                            </div>
                        </div>
                    </div>
