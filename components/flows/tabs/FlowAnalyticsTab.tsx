...
const FlowAnalyticsTab: React.FC<{ flow: Flow }> = memo(({ flow }) => {
    ...
    // CHUY?N Ð?I D? LI?U TH?C T? SANG FUNNEL (REAL-TIME LOGIC)
    const funnelData = useMemo(() => {
        ...
        // Filter for Action Steps from the SORTED list - ADD zalo_zns
        ...
        const traverse = (stepId: string) => {
            ...
            if (nextStepId) {
                ...
                finalFunnel.push({
                    ...
                    waiting: 0,
                    rate: 100,
                    detailStat: `Hoàn t?t: ${completedCount}`,
                    config: {},
    ...
    const calculateTimeRemaining = (dateStr: string) => {
        ...
        if (diff <= 0) return 'S?p ch?y ngay';
    ...
    const handleExportReport = async () => {
        showToast('Ðang xu?t báo cáo...', 'info');
        const res = await api.post(`flows?id=${flow.id}&route=export-analytics`, {});
        if (res.success) {
            showToast('Xu?t báo cáo thành công!');
        } else {
            showToast('L?i khi xu?t báo cáo', 'error');
        }
    };
    ...
    const fetchInactiveUsers = async (page = 1) => {
        ...
        try {
            ...
            if (res.success) {
                ...
                showToast('Không th? t?i danh sách', 'error');
            }
        } catch (err) {
            console.error(err);
    ...
    const handleCleanFailedUsers = () => {
        setConfirmModal({
            isOpen: true,
            title: 'Xác nh?n d?n d?p',
            message: 'B?n có ch?c mu?n xóa t?t c? ngu?i dùng b? l?i kh?i flow này?',
            onConfirm: async () => {
                setConfirmModal({ ...confirmModal, isOpen: false });
                showToast('Ðang d?n d?p...', 'info');
                const res = await api.post(`flows?id=${flow.id}&route=clean-failed`, {});
                if (res.success) {
                    showToast('Ðã d?n d?p ngu?i dùng b? l?i!');
                    refreshFlow();
                } else {
                    showToast('L?i khi d?n d?p', 'error');
                }
            }
        });
    };
    ...
    return (
        <div className="p-3 md:p-6 lg:px-8 lg:pb-8 lg:pt-4 space-y-4 md:space-y-8 max-w-[1400px] mx-auto animate-in fade-in duration-500 relative pb-20">
                    ...
                    <div className="lg:col-span-5 lg:border-r border-white/10 lg:pr-10">
                        <div className="flex items-center justify-between mb-6 md:mb-8">
                            <div className="flex items-center gap-3 md:gap-4">
                                ...
                                <div>
                                    <h3 className="text-lg md:text-2xl font-bold tracking-tight">Hi?u su?t v?n hành</h3>
                                    <div className="flex items-center gap-2 mt-0.5 md:mt-1">
                                        ...
                                        <span className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                                            Automation {currentFlow.status === 'active' ? 'dang ch?y' : 'dã d?ng'}
                                        </span>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="space-y-4 md:space-y-6">
                            <div className="flex justify-between items-end">
                                <div>
                                    <p className="text-[9px] md:text-[10px] font-bold uppercase text-slate-500 tracking-widest mb-1">T? l? hoàn t?t</p>
                                ...
                                <div className="text-right">
                                    <p className="text-[9px] md:text-[10px] font-bold uppercase text-slate-500 tracking-widest mb-1">T? l? m? TB</p>
                    ...
                    <div className="lg:col-span-7 flex flex-col gap-6">
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8">
                            <div className="bg-white/5 p-3 rounded-xl border border-white/5 lg:bg-transparent lg:p-0 lg:border-0">
                                <p className="text-[8px] md:text-[9px] font-bold uppercase text-slate-500 tracking-widest mb-1.5 md:mb-2 text-center lg:text-left">Ðã g?i</p>
                            ...
                            <div className="bg-white/5 p-3 rounded-xl border border-white/5 lg:bg-transparent lg:p-0 lg:border-0">
                                <p className="text-[8px] md:text-[9px] font-bold uppercase text-slate-500 tracking-widest mb-1.5 md:mb-2 text-center lg:text-left">Ðã m?</p>
                                <div className="flex items-center justify-center lg:justify-start gap-2">
                                    <div className="p-1 px-1.5 bg-white/5 rounded-lg border border-white/5 hidden md:block">
                            ...
                            <div className="bg-white/5 p-3 rounded-xl border border-white/5 lg:bg-transparent lg:p-0 lg:border-0">
                                <p className="text-[8px] md:text-[9px] font-bold uppercase text-slate-500 tracking-widest mb-1.5 md:mb-2 text-center lg:text-left">G?i l?i</p>
                                <div className="flex items-center justify-center lg:justify-start gap-2">
                                    <div className="p-1 px-1.5 bg-white/5 rounded-lg border border-white/5 hidden md:block">
                            ...
                            <div className="bg-white/5 p-3 rounded-xl border border-white/5 lg:bg-transparent lg:p-0 lg:border-0">
                                <p className="text-[8px] md:text-[9px] font-bold uppercase text-slate-500 tracking-widest mb-1.5 md:mb-2 text-center lg:text-left">H?y ÐK</p>
                                <div className="flex items-center justify-center lg:justify-start gap-2">
                                    <div className="p-1 px-1.5 bg-white/5 rounded-lg border border-white/5 hidden md:block">
                        ...
                        {triggerStep?.config?.type === 'campaign' && triggerStep.config.targetId && (
                            <div className="flex items-center gap-4 p-3 md:p-4 bg-white/5 border border-white/10 rounded-xl md:rounded-2xl backdrop-blur-sm animate-in fade-in slide-in-from-bottom-4 duration-700">
                                ...
                                <div className="flex-1">
                                    <p className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase tracking-widest">Chi?n d?ch g?c</p>
                                    <p className="text-[10px] md:text-xs font-bold text-slate-200">Truy c?p báo cáo chi ti?t</p>
                                </div>
                                <button
                        ...
                        <div className="p-4 md:p-8">
                            {funnelData.length > 0 ? (
                                                    ...
                                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between w-full gap-4">
                                                        ...
                                                        {item.isTrulyStuck && (
                                                            <BottleneckBadge
                                                                type="time"
                                                                value="Stuck > 24h"
                                                                tooltip="C?nh báo: Khách hàng dang b? ngh?n t?i bu?c này quá 24h so v?i th?i gian x? lý d? ki?n. B? qua c?nh báo này n?u logic c?a b?n là dúng."
                                                            />
                                                        )}
                                                        {item.dropOffRate > 0.7 && (
                                                            <BottleneckBadge
                                                                ...
                                                                tooltip="C?nh báo: T? l? thoát/d?ng t?i bu?c này cao (>70%). B?n nên t?i uu n?i dung ho?c di?u ki?n l?c."
                                                            />
                                                        )}
                                                        ...
                                                        <div className="flex items-center justify-between sm:justify-end gap-4 md:gap-8 text-right">
                                                            <div className="flex gap-4 md:gap-6">
                                                                {item.type === 'completed' ? (
                                                                    ...
                                                                    <>
                                                                        <div>
                                                                            <p className="text-[8px] md:text-[9px] text-slate-400 font-bold uppercase tracking-wide whitespace-nowrap">Ðang ? dây</p>
                                                                            <p className="text-base md:text-lg font-black text-slate-800">{(item as any).waiting?.toLocaleString() || 0}</p>
                                                                        </div>
                                                                        <div>
                                                                            <p className="text-[8px] md:text-[9px] text-slate-400 font-bold uppercase tracking-wide whitespace-nowrap">Ðã di qua</p>
                                ...
                                <div className="text-center py-20 opacity-40">
                                    ...
                                    <p className="text-xs font-bold uppercase tracking-widest">Chua có d? li?u v?n hành</p>
                                </div>
                            )}
                        </div>
                    </Card >
                        ...
                        <div className="px-4 py-3 border-b border-slate-50 bg-slate-50/30 backdrop-blur-sm sticky top-0 z-10">
                            <div className="flex justify-between items-center">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-[10px] font-black text-slate-700 uppercase tracking-widest">Nh?t ký</span>
                                </div>
                        ...
                        <div className="flex-1 overflow-y-auto max-h-[600px] p-0 scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent">
                            {loadingLogs ? (
                                ...
                                <div className="flex flex-col items-center justify-center py-12 opacity-40">
                                    ...
                                    <p className="text-[10px] font-bold uppercase text-slate-400">Chua có s? ki?n nào</p>
                                </div>
                            ) : (
                            ...
                            <div className="px-4 py-3 border-t border-slate-100 bg-slate-50/30">
                                <div className="flex items-center justify-between">
                                    <div className="text-[9px] text-slate-500 font-bold">
                                        Trang {logPagination.page} / {logPagination.totalPages} • T?ng {logPagination.total} s? ki?n
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <button
            ...
            <StepErrorModal
                ...
                onExport={() => showToast('Ðã xu?t danh sách l?i')}
            />
            ...
            <StepUnsubscribeModal
                ...
                onExport={() => showToast('Ðã xu?t danh sách h?y dang ký')}
            />
            ...
            <StepParticipantsModal
                ...
                title={selectedStatus === 'completed' ? 'Danh sách hoàn thành' : `Danh sách t?i: ${selectedStepId ? getStepName(selectedStepId) : '...'}`}
            ...
            <StepParticipantsModal
                ...
                title="KHÔNG TUONG TÁC (Inactive)"
