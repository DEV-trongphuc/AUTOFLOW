 ...
 * Giúp MySQL tìm th?y b?n ghi k? c? khi user dùng t? khác (VD: "m?c không" -> "giá cost")
 */
function expandQueryWithSynonyms($query, $customSynonyms = [])
{
    ...
    $synonyms = [
        // ===== GIÁ / TI?N =====
        'giá' => ['chi phí', 'bao nhiêu', 't?n kém', 'm?c', 'd?t', 'r?', 'vnd', 'vnd', 'd?ng', 'ti?n', 'báo giá', 'price', 'cost', 'h?c phí', 'phí', 'fee', 'budget'],
        // ===== MUA / ÐANG KÝ =====
        'mua' => ['d?t hàng', 'order', 'thanh toán', 's? h?u', 'dang ký', 'ghi danh', 'apply', 'enroll', 'buy', 'purchase', 'sign up', 'dóng ti?n', 'ck'],
        // ===== ? ÐÂU / Ð?A CH? =====
        '? dâu' => ['d?a ch?', 'v? trí', 'ch? nào', 't?a d?', 'map', 'location', 'address', 'noi', 'co s?', 't?i dâu'],
        // ===== LIÊN H? =====
        'liên h?' => ['g?i', 'sdt', 'hotline', 'email', 'nh?n tin', 'inbox', 'contact', 'h? tr?', 'tu v?n', 'chat', 'zalo'],
        // ===== ÐÁNH GIÁ =====
        't?t không' => ['review', 'dánh giá', 'feedback', 'ch?t lu?ng', 'uy tín', 'có nên', 'ok không', '?n không', 'scam'],
        // ===== HU?NG D?N =====
        'hu?ng d?n' => ['cách làm', 'làm sao', 'th? nào', 'các bu?c', 'quy trình', 'guide', 'tutorial', 'how to'],
        // ===== KHUY?N MÃI =====
        'khuy?n mãi' => ['uu dãi', 'gi?m giá', 'voucher', 'code', 'quà t?ng', 'bonus', 'deal', 'sale', 'free'],
        // ===== TH?I GIAN =====
        'khi nào' => ['th?i gian', 'khai gi?ng', 'l?ch h?c', 'h?n chót', 'deadline', 'm? khóa', 'b?t d?u', 'bao gi?'],
        // ===== ÐI?U KI?N / TRÌNH Ð? =====
        'di?u ki?n' => ['yêu c?u', 'd?u vào', 'c?n gì', 'b?ng c?p', 'kinh nghi?m', 'ielts', 'ti?ng anh', 'ngo?i ng?', 'english', 'trình d?', 'nang l?c', 'level'],
        'ti?ng anh' => ['english', 'ngo?i ng?', 'ielts', 'toeic', 'toefl', 'vstep', 'b1', 'b2', 'giao ti?p'],
        'y?u' => ['kém', 'chua t?t', 'm?t g?c', 'không bi?t', 'lâu r?i không dùng', 'ch?m', 'th?p', 'n? b?ng', 'thi?u'],
    ];
...
function classifyQuery($query)
{
    ...
    $patterns = [
        'social' => '/^(chào|hi|hello|helo|hê lô|hey|alo|ê|hoi|h?i|cho h?i|chúc|c?m on|thanks|t?m bi?t|bye|ok|vâng|d?|?a|?|yes|no)/ui',
        'price' => '/giá|bao nhiêu|chi phí|phí|ti?n|cost|price/ui',
        'howto' => '/làm sao|cách|hu?ng d?n|th? nào|how to|how do/ui',
        'comparison' => '/khác|so sánh|hon|t?t hon|vs|versus|compare/ui',
        'factual' => '/là gì|d?nh nghia|nghia là|what is|define/ui',
        'location' => '/? dâu|d?a ch?|noi nào|where|location/ui',
        'timing' => '/(?:^|[^a-z0-9àá??ãâ??y???a?????èé???ê?????ìí??iòó??õô?????o?????ùú??uu??????ý???])(khi nào|lúc nào|bao gi?|m?y gi?|th?i di?m|l?ch|ngày|tháng|nam|deadline|k?|khai gi?ng|when|time|schedule|deadline)(?![a-z0-9àá??ãâ??y???a?????èé???ê?????ìí??iòó??õô?????o?????ùú??uu??????ý???])/ui',
    ];
...
function retrieveContext($pdo, $propertyId, $userMsg, $contextParams, $apiKey, $limit = 20)
{
    ...
    // ? OPTIMIZATION: Ch? lo?i b? các ch? d?n c? th? trong ngo?c (VD: [Tr? l?i ng?n g?n])
    // Tránh nu?t m?t text quan tr?ng c?a ngu?i dùng.
    $cleanUserMsg = preg_replace('/\[(tr? l?i|ng?n g?n|chi ti?t|b?ng ti?ng|ngôn ng?).*?\]/ui', '', $userMsg);
    ...
    if (!empty($topIds)) {
        ...
        foreach ($stmtFull->fetchAll(PDO::FETCH_ASSOC) as $row) {
            ...
            if ($citeMode) {
                ...
                // if (!empty($citations)) {
                //     $row['content'] = "[" . implode(" | ", $citations) . "]\n$formattedContent\n(Note: B?n B?T BU?C ph?i trích d?n ngu?n b?ng format [Trang X](link_file_goc) n?u s? d?ng thông tin này)";
                // }
            }
    ...
    if (empty($intentConfigs)) {
        $intentConfigs = [
            ['name' => 'price', 'regex' => '(?:^|[^a-z0-9àá??ãâ??y???a?????èé???ê?????ìí??iòó??õô?????o?????ùú??uu??????ý???])(giá|phí|ti?n|vnd|bao nhiêu|nhiêu|d|k)(?![a-z0-9àá??ãâ??y???a?????èé???ê?????ìí??iòó??õô?????o?????ùú??uu??????ý???])', 'boost' => 1.3],
            ['name' => 'howto', 'regex' => '(?:^|[^a-z0-9àá??ãâ??y???a?????èé???ê?????ìí??iòó??õô?????o?????ùú??uu??????ý???])(cách|làm sao|hu?ng d?n|bu?c|quy trình|th? t?c)(?![a-z0-9àá??ãâ??y???a?????èé???ê?????ìí??iòó??õô?????o?????ùú??uu??????ý???])', 'boost' => 1.25],
            ['name' => 'location', 'regex' => '(?:^|[^a-z0-9àá??ãâ??y???a?????èé???ê?????ìí??iòó??õô?????o?????ùú??uu??????ý???])(d?a ch?|? dâu|v? trí|map|noi|t?a d?)(?![a-z0-9àá??ãâ??y???a?????èé???ê?????ìí??iòó??õô?????o?????ùú??uu??????ý???])', 'boost' => 1.2],
            ['name' => 'timing', 'regex' => '(?:^|[^a-z0-9àá??ãâ??y???a?????èé???ê?????ìí??iòó??õô?????o?????ùú??uu??????ý???])(khi nào|lúc nào|bao gi?|l?ch|ngày|tháng|deadline|khai gi?ng)(?![a-z0-9àá??ãâ??y???a?????èé???ê?????ìí??iòó??õô?????o?????ùú??uu??????ý???])', 'boost' => 1.5],
        ];
    }
    ...
    if ($qLen < 40) {
        ...
        $hasProperNouns = preg_match('/[A-ZÐ][a-zàáâãèéêìíòóôõùúýadiuou?????????????????????????????????????????????]{2,}/u', $userMsg);
    }
    ...
    foreach ($scoredResults as $row) {
        ...
        // Uu tiên doc m?i c?p nh?t hon khi n?i dung tuong t? (VD: h?c phí v1 vs h?c phí v2)
        $recencyBoost = 1.0;
        if (!empty($row['doc_updated_at'])) {
            ...
            if ($updatedTs > 0) {
                ...
                if ($daysSinceUpdate <= 7) {
                    $recencyBoost = 1.30; // C?p nh?t trong 7 ngày: boost m?nh
                } elseif ($daysSinceUpdate <= 30) {
                    $recencyBoost = 1.15; // Trong 30 ngày: boost v?a
                } elseif ($daysSinceUpdate <= 90) {
                    $recencyBoost = 1.05; // Trong 90 ngày: boost nh?
                }
