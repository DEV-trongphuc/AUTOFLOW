...
body { font: 13px 'Segoe UI', monospace; background: #0d1117; color: #e4e4e7; padding: 16px; }
h1 { color: #a78bfa; font-size: 18px; margin-bottom: 4px; }
h2 { color: #60a5fa; font-size: 14px; margin: 18px 0 6px; border-left: 3px solid #60a5fa; padding-left: 8px; }
h3 { color: #94a3b8; font-size: 12px; margin: 12px 0 4px; }
.meta { color: #6b7280; font-size: 11px; margin-bottom: 16px; }
.card { background: #161b22; border: 1px solid #21262d; border-radius: 8px; padding: 12px; margin-bottom: 10px; }
.ok  { color: #3fb950; } .warn { color: #d29922; } .err { color: #f85149; } .info { color: #79c0ff; }
table { width: 100%; border-collapse: collapse; font-size: 12px; }
th { background: #21262d; color: #8b949e; padding: 5px 8px; text-align: left; }
td { padding: 4px 8px; border-bottom: 1px solid #161b22; vertical-align: top; }
tr:hover td { background: #1c2128; }
.badge { display: inline-block; padding: 1px 7px; border-radius: 3px; font-size: 11px; font-weight: 600; }
.b-wait   { background: #1f3656; color: #79c0ff; }
.b-action { background: #1a3a2a; color: #3fb950; }
.b-cond   { background: #3d2e00; color: #d29922; }
.b-trig   { background: #2d1b4e; color: #d2a8ff; }
.b-waiting    { background: #1f3656; color: #79c0ff; }
.b-processing { background: #3d2e00; color: #d29922; }
.b-completed  { background: #1a3a2a; color: #3fb950; }
.b-failed     { background: #3d1515; color: #f85149; }
.alert { padding: 8px 12px; border-radius: 6px; margin: 8px 0; line-height: 1.6; font-size: 12px; }
.alert-ok   { background: #1a3a2a; border-left: 3px solid #3fb950; }
.alert-warn { background: #2d2008; border-left: 3px solid #d29922; }
.alert-err  { background: #2d0f0f; border-left: 3px solid #f85149; }
.alert-info { background: #1f3656; border-left: 3px solid #79c0ff; }
code { background: #21262d; padding: 1px 5px; border-radius: 3px; font-size: 11px; }
pre  { background: #0d1117; border: 1px solid #21262d; padding: 10px; border-radius: 5px; overflow-x: auto; font-size: 11px; color: #e4e4e7; }
.step-box { border: 1px solid #21262d; border-radius: 6px; padding: 10px; margin: 6px 0; }
.step-wait   { border-color: #1f3656; }
.step-action { border-color: #1a3a2a; }
.step-cond   { border-color: #3d2e00; }
.sim-row { display: flex; gap: 10px; margin: 4px 0; }
.sim-label { color: #8b949e; min-width: 140px; flex-shrink: 0; }
.sim-val { flex: 1; }
a { color: #79c0ff; }
.fix-btn { background: #388bfd; color: #fff; border: none; padding: 4px 12px; border-radius: 4px; cursor: pointer; font-size: 11px; text-decoration: none; display: inline-block; margin: 4px 0; }
.fix-btn-red { background: #f85149; }
</style>
</head>
<body>
<?php
...
if (!$flowRow) { echo "<div class='alert alert-err'>❌ Flow not found</div>"; exit; }

$flowSteps = json_decode($flowRow['steps'], true) ?? [];
$stepMap   = [];
foreach ($flowSteps as $s) $stepMap[$s['id']] = $s;
...
$fields = ['id'=>'Queue ID','subscriber_id'=>'Sub ID','status'=>'Status','step_id'=>'Step ID în DB',
           'scheduled_at'=>'scheduled_at','updated_at'=>'updated_at','created_at'=>'created_at',
           'last_step_at'=>'last_step_at','last_error'=>'last_error'];
...
if (empty($actLog)) {
    ...
    foreach ($actLog as $i => $act) {
        ...
        if ($prevTime) {
            ...
            if ($diff < 60) $gap = "<span class='err'>{$diff}s</span>";
            elseif ($diff < 3600) $gap = "<span class='warn'>" . round($diff/60, 1) . "m</span>";
            else $gap = "<span class='info'>" . round($diff/3600, 1) . "h</span>";
        }
...
if (!$canBePickedUp) {
    if ($state['status'] === 'waiting') {
        ...
        echo "<div class='alert alert-ok'>✅ Subscriber đang chờ đúng. Sẽ được xử lý sau <b>{$hoursLeft}h</b> (lúc $simItemScheduledAt).</div>";
    }
    echo "</div>";
    ...
    if ($type === 'wait') {
        ...
        if ($isResumedWait) {
            ...
            if ($mode === 'until_date') {
                ...
                if (!$isWaitOver) $nextScheduledAt = date('Y-m-d H:i:s', $targetTs);
                ...
                if ($dt->getTimestamp() <= $simNowTs) $dt->modify("+1 day");
            ...
            if ($isWaitOver) {
                ...
                $nextId = $s['nextStepId'] ?? null;
                if (!$nextId) { echo "<div class='alert alert-ok'>🏁 Flow COMPLETED!</div>"; break; }
                $chainStepId = $nextId;
                echo "</div></div>";
                continue;
            } else {
...
$actionStepCount = 0;
foreach ($chain as $_s) {
    if ($_s['type'] === 'action') $actionStepCount++;
}
...
foreach ($chain as $idx => $s) {
    if ($s['id'] === $currentStepId) { $currentStepInChain = $idx; break; }
}
